...
void H2P_build_FSAI_precond(H2Pack_p h2pack, const int rank, const DTYPE shift, FSAI_precond_p *precond_)
{
    ...
    size_t total_msize = 2.0 * ((sizeof(DTYPE) + sizeof(int)) * (nnz + mat_size));
...
void FSAI_precond_apply(FSAI_precond_p precond, const DTYPE *b, DTYPE *x)
{
    ...
    double st = get_wtime_sec();
    gather_vector_elements(sizeof(DTYPE), precond->mat_size, precond->fwd_pmt, b, precond->pmt_b);
    CSRP_SpMV(precond->G,  precond->pmt_b, precond->x0);
    CSRP_SpMV(precond->Gt, precond->x0, precond->pmt_x);
    gather_vector_elements(sizeof(DTYPE), precond->mat_size, precond->bwd_pmt, precond->pmt_x, x);
    double et = get_wtime_sec();
