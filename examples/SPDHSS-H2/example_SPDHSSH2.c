 ...
 *  
 *  Example run: 
 *  ./example_SPDHSSH2.exe 3 80000 1e-8 1 1 ./point_set/3Dball_80000.csv 0.1 1e-2
 *  Input: 
 *      3 --> point dimension
...
int main(int argc, char **argv)
{
    ...
    //  PCG with FSAI 
    int fsai_rank = 200;
    pcg_tests(test_params.krnl_mat_size, h2mat, hssmat, shift, fsai_rank, max_iter, CG_tol, 4);
    printf("FSAI rank setting: r = %d\n", fsai_rank);
    fflush(stdout);

    //  PCG with BJ
    pcg_tests(test_params.krnl_mat_size, h2mat, hssmat, shift, 0, max_iter, CG_tol, 2);
    fflush(stdout);
