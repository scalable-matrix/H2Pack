...
int main(int argc, char **argv)
{
    ...
    printf("Generating random coordinates in a unit cubic box...\n");
    ...
    {
        st = get_wtime_sec();
        double max_L = h2pack->root_enbox[pt_dim];
        H2P_generate_proxy_point_ID(
            pt_dim, krnl_dim, rel_tol, h2pack->max_level, h2pack->min_adm_level,
            max_L, krnl_param, krnl_eval, &pp
        );
        et = get_wtime_sec();
    }
    #endif
    ...
    H2P_build(
        h2pack, pp, BD_JIT, krnl_param, krnl_eval, krnl_bimv, krnl_bimv_flops
    );
